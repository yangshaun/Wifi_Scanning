...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		timetext.setText("Current Time : "
				+ dateFormat.format(time_now).toString() + "\n"
				+ "Your Device ID :" + android_id);
		String[] options_str = {"3分鐘", "5分鐘", "10分鐘", "15分鐘"};
		options = new ArrayAdapter<String>(this, R.layout.spin_item,
				options_str);
		...
		if (isMyServiceRunning(Wifi_service.class)) {
			...
			Toast.makeText(MainActivity.this, "Service 正在掃描中....",
		...
		buttonScan.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!locationenable.isProviderEnabled(LocationManager.GPS_PROVIDER)){
					 ...
					 Toast.makeText(getBaseContext(), "嘿你幫我開一下GPS吧!!!甘溫阿^^",
				...
				if (isMyServiceRunning(Wifi_service.class)) { // 又有service再跑
					...
					Toast.makeText(MainActivity.this, "已經開始掃描了唷!!",
							Toast.LENGTH_SHORT).show();
				}

				Log.e("QWQWQWQWQWQWQW", "Outside!!!!!!!!!!!!!!!!!!!"
						+ " current : " + current_period + " period : "
						+ period);
		...
		update.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				if (isMyServiceRunning(Wifi_service.class)) {
					AndroidAuthSession session = Dropbox_initial.dropbox
							.getSession();
					if (session.authenticationSuccessful()) {
						intent = new Intent(MainActivity.this, wifiservice
								.getClass());
						stopService(intent);
						progress.setVisibility(1);
						dropbox.onClick(1);
	...
	protected void onResume() {
		...
		if (isMyServiceRunning(Wifi_service.class)) {
			...
			Toast.makeText(this,
					"目前已經收集到" + wifi_num.getInt("wifi_num",0)+ "筆資料了!",
					Toast.LENGTH_SHORT).show();
			dropbox.onResume();

		}else{
			Toast.makeText(this,"目前尚未開始掃描!請按下開始鈕!",
					Toast.LENGTH_SHORT).show();
		}
